// newcalibration.cpp : Defines the entry point for the console application.
//
...
for(i=0;i < (height);i++) 
{
	for(j=0;j <(width);j++)
	{
		if(((data[i*step+j*channels+1]) > (7+data[i*step+j*channels]))
		&& ((data[i*step+j*channels+1]) > (30+data[i*step+j*channels+2])))
